        ...
        <div class="container-fluid px-4 px-md-5">
            <div class="row mb-5 text-center">
                <div class="col-12">
                    ...
                    <p class="fs-5 text-secondary">Explore our range of exceptional timepieces, from timeless classics
                        to avant-garde complications.</p>
                </div>
            </div>
            ...
            <div class="row mb-5 align-items-center border-bottom border-secondary pb-4">
                <div class="col-lg-8">
                    <div class="d-flex flex-wrap gap-3 align-items-center" id="filterButtons">
                        ...
                        <button class="btn btn-sm btn-outline-secondary text-secondary-bold px-3 active"
                            data-filter="all">ALL</button>
                        <button class="btn btn-sm btn-outline-secondary text-secondary-bold px-3"
                            data-filter="rolex">ROLEX</button>
                        <button class="btn btn-sm btn-outline-secondary text-secondary-bold px-3"
                            data-filter="cartier">CARTIER</button>
                        <button class="btn btn-sm btn-outline-secondary text-secondary-bold px-3"
                            data-filter="montblac">MONTBLAC</button>
                        <button class="btn btn-sm btn-outline-secondary text-secondary-bold px-3"
                            data-filter="patek philippe">PATEK PHILIPPE</button>
                    </div>
                </div>
                <div class="col-lg-4 text-lg-end mt-3 mt-lg-0">
                    <div class="d-flex justify-content-lg-end align-items-center gap-2">
                        ...
                        <select id="sortSelect" class="form-select form-select-sm bg-black border-secondary text-white"
                            style="width: auto;">
                ...
                <?php if (!empty($products)): ?>
                    ...
                    foreach ($products as $product): ?>
                        ...
                        <div class="col-12 col-sm-6 col-lg-3 product-item"
                            data-category="<?= htmlspecialchars(strtolower($product['category'] ?? ''), ENT_QUOTES) ?>"
                            data-price="<?= htmlspecialchars($product['price'], ENT_QUOTES) ?>" data-index="<?= $i ?>">
                            <div class="product-card rounded-3 p-3 h-100 d-flex flex-column justify-content-between"
                                ...
                                <a href="viewProduct.php?id=<?= htmlspecialchars($product['id'], ENT_QUOTES) ?>"
                                    ...
                                    <div>
                                        ...
                                        <p class="text-secondary small text-uppercase">
                                            <?= htmlspecialchars($product['category'], ENT_QUOTES) ?></p>
                                        <h5 class="text-white text-secondary-bold">
                                            <?= htmlspecialchars($product['name'], ENT_QUOTES) ?></h5>
                                        <p class="text-white mb-1"><?= formatPrice($product['price']) ?></p>
                                        <p class="text-secondary small mb-1 product-stock"
                                            data-product-id="<?= htmlspecialchars($product['id'], ENT_QUOTES) ?>">Stock: <span
                                                class="stock-count"><?= (int) ($product['stock'] ?? 0) ?></span></p>
                                    </div>
                                </a>
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <button class="btn btn-sm btn-outline-light flex-fill add-to-cart-btn" type="button"
                                        ...
                                        data-restore-label="ADD TO CART" <?= $inStock ? '' : 'disabled' ?>>
